...
Knight::Knight(Board *next, int x, int y, char team) : next{next}, x{x}, y{y}, team{team}, alive{true} {}
...
{
    if (x1 == x && y1 == y && team == 'w' && alive)
        return 'n';
    else if (x1 == x && y1 == y && alive)
        return 'N';
    else
        return next->getTile(x1, y1);
}
