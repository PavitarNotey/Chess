...
Game::Game(Board **b, GameManager *gm, Player *player1, Player *player2) : move{0}, head{b}, gm{gm}, stack{}, white_king{-1, -1}, black_king{-1, -1}, player1{player1}, player2{player2} {}
...
std::pair<int, int> Game::getCoords()
{
    ...
    if (std::cin >> inp)
    {
        ...
        res.first = inp[0] - 'a';
        res.second = inp[1] - '1';
        if (0 > res.first || res.first > 7)
            res.first = -1;
        if (0 > res.second || res.second > 7)
            res.second = -1;
    }
    else
    {
...
void Game::removePiece()
{
    ...
    if (coords.first != -1 && coords.second != -1)
    {
        ...
        if ('a' <= tile && tile <= 'z' && this->isCheck('w'))
            (*head)->setAlive(coords.first, coords.second);
        else if ('A' <= tile && tile <= 'Z' && this->isCheck('b'))
            (*head)->setAlive(coords.first, coords.second);
    }
    else
        out << "Invalid command " << inp << " use --help for more options" << std::endl;
}
    ...
    if (std::cin >> inp)
    {
        ...
        if (inp == "K")
        {
            ...
            if (coords.first != -1 && coords.second != -1)
            {
                ...
                {
                    out << "A piece already occupies this tile" << std::endl;
                }
            }
            else
            {
...
bool Game::pawnValidMove(int x1, int y1, int x2, int y2)
{
    ...
    if (team == 'b')
        dir = -1;
    if ((x2 - x1 == 0) && (y1 + dir + dir == y2) && (*head)->getFirstMove(x1, y1))
    { // double pawn first mov
        this->setDead(x2, y2);
    ...
    else if ((x2 - x1 == 1 || x2 - x1 == -1) && (y1 + dir == y2) && ((*head)->getTile(x2, y2) != ' '))
    { // diagonal pawn move
        this->setDead(x2, y2);
        (*head)->move(x1, y1, x2, y2);
        if (this->isCheck(team))
...
bool Game::kingValidMove(int x1, int y1, int x2, int y2, bool castle)
{
    ...
    if (castle && (y1 == y2) && (*head)->getFirstMove(x1, y1) && (*head)->getFirstMove(x2, y2))
    { // handles castling
        int dir = -1;
        if (newX < 0)
            dir = 1;
